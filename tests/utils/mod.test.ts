...
import {
    toBitstream, fromBitstream, toHex, fromHex, calcCsum, verifyCsum,
    padBytes, padBitstream, bitstreamToMs, msToBitstream, msToSeed, msToSeedAsync
...
Deno.test(`Util function 'toBitstream' converts a Uint8Array to a bitstream`, () => {
    const bytes: Uint8Array = Uint8Array.from([9, 8, 7, 6]); // Create Uint8Array
...
/**
 * Test for 'toHex' util function.
 * 
 * Converts a Uint8Array to a hexadecimal string. Assigns the expected result to a 'control' const.
 * 
 * Console.assert checks that the hexadecimal string matches the control.
 */
Deno.test(`Util function 'toHex' converts a Uint8Array to a hex string`, () => {
    const bytes: Uint8Array = Uint8Array.from([11, 19, 13, 17]); // Create Uint8Array
    const hex = toHex(bytes); // Convert Uint8Array to hex string
    const control = "0b130d11"; // Expected conversion result as the control
    console.assert(hex === control, `Util function 'toHex' failed to convert a Uint8Array to a hex string`);
});

/**
 * Test for 'fromHex' util function.
 * 
 * Converts a hexadecimal string to a Uint8Array. Assigns the expected result to a 'control' const.
 * 
 * Console.assert checks that the Uint8Array matches the control.
 */
Deno.test(`Util function 'fromHex' converts a hex string to a Uint8Array`, () => {
    const hex: string = "0b130d11"; // Define hex string
    const bytes = fromHex(hex); // Convert hex string to Uint8Array
    const control = Uint8Array.from([11, 19, 13, 17]); // Expected conversion result as the control
    console.assert(bytes.every((byte, i) => byte === control[i]), `Util function 'fromHex' failed to convert a hex string to a Uint8Array`);
});
...
Deno.test(`Util function 'msToSeed' creates a seed from a mnemonic sentence (synchronous)`, () => {
    ...
    console.assert(toHex(seed) === control, `Util function 'msToSeed' failed to create a seed from a mnemonic sentence`);
});
...
Deno.test(`Util function 'msToSeedAsync' creates a seed from a mnemonic sentence (asynchronous)`, async () => {
    ...
    // Expected seed represented as a hex string as the control
    const control = "ae290acb10004461d3e3c07c4e534a6619e68524f401bdbcaa8f0d6c02509645cb2221aaf238578c59c218e090662ba326d8182839a20499404c6a8431a430ed";
    console.assert(toHex(seed) === control, `Util function 'msToSeedAsync' failed to create a seed from a mnemonic sentence`);
});
