...
export function calcCsum(bits: string): string {
    const ent = bits.length; // Get the initial entropy length (ENT)
    ...
    const cs = ent / 32; // Determine checksum length (CS)
    ...
    const csum = toBitstream(hash).slice(0, cs); // Extract checksum of length 'cs' from the hash
...
export function verifyCsum(bits: string): boolean {
    ...
    const entLen = (bitLen * 32) / 33; // Determine the ENT length
